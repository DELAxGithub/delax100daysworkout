...
#### 1. 自動プルシステム (`scripts/auto-pull.sh`) ⭐ NEW
```bash
# 使用方法
./scripts/auto-pull.sh start          # 監視開始（バックグラウンド）
./scripts/auto-pull.sh once           # 一回だけチェック実行
./scripts/auto-pull.sh stop           # 監視停止
./scripts/auto-pull.sh status         # 監視状態確認
```
...
#### 2. PR同期スクリプト (`scripts/sync-pr.sh`)
```bash
# 使用方法（任意利用）
...
#### 3. 通知システム (`scripts/notify.sh`) 🔄 UPDATED
```bash
# 使用方法
